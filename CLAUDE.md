...
- **External dependencies in tests** (R2, Gemini, the Google OAuth client) are monkeypatched at the exact boundary the app code calls through, not mocked deeper - see `test_upload.py` or `test_integrations.py` for the pattern. Everything on the FastAPI side of that boundary runs for real, including the database.
...
- `backend-py/.env` — `DATABASE_URL`, `REDIS_URL`, `SECRET_KEY` (signs/verifies JWTs), `JWT_ALGORITHM`, `ACCESS_TOKEN_EXPIRE_MINUTES`, `ENCRYPTION_KEY`, `FRONTEND_URL`, `R2_*`, `RESEND_*`, `GEMINI_API_KEY`, `GOOGLE_SERVICE_ACCOUNT_JSON`, `GOOGLE_CALENDAR_ID`, `GOOGLE_OAUTH_*`
